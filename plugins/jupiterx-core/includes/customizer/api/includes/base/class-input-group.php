...
class JupiterX_Customizer_Base_Input_Group extends JupiterX_Customizer_Base_Control {
	...
	protected function group_prefix_template() {
		...
		<# } else if ( hasIcon ) { #>
			<span class="jupiterx-input-group-icon">
				<img <# if ( ! _.isEmpty( data.alt ) ) { #> title="{{ data.alt }}" <# } #> src="<?php echo esc_url( JupiterX_Customizer_Utils::get_assets_url() ); ?>/img/{{ data.icon }}.svg" />
			</span>
		<# } #>
		<?php
	}

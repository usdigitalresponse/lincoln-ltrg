...
if ( ! class_exists( 'JupiterX_Control_Panel_Export_Import' ) ) {
	...
	class JupiterX_Control_Panel_Export_Import {
		...
		private function _export_settings_options( $data ) {
			...
			// Smart Filter.
			$jet_filter_conditions = get_option( 'Jet_Smart_Filters' );

			if ( ! empty( $jet_popup_conditions ) && in_array( 'jet-smart-filters', $data['options']['jupiterx_support_plugins'], true ) ) {
				$data['options']['Jet_Smart_Filters'] = $jet_filter_conditions;
			}
